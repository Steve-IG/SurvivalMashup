...
- **Melee feel & impact readability (Milestone 2, Review Group 5C).** The punch was made snappy rather than floaty and its cues made unambiguous — no new mechanics. `PlayerCombat` now reports **three distinct stages** — `Attacked` (swing started) → `Contacted` (contact frame, hit or miss) → `Impacted` (hit **confirmed**) — and **all** impact feedback (camera impulse, VFX, hit-stop, sound) is wired to `Impacted` alone, so swinging into empty air produces no impact cue; the old swing-start camera shake was removed. **Latency removed:** post-impact commitment is now its own knob (`_postImpactCommit`, 0.05s) separate from swing recovery (0.35s → 0.12s), so movement returns almost immediately after contact instead of 0.35s later (measured: committed 0.75s → 0.53s). **Impact position** now comes from the real `HitResult.ContactPoint` (`Collider.ClosestPoint` from the attacking hand) rather than a chest-height/inset approximation. No combos and no animation cancelling were added — only latency was removed.
- **Player health bar (Milestone 2, Review Group 5C).** A thin presentation adapter (`PlayerHealthBar`) shows Current Health on screen: it fills to `Current / Maximum`, so it lowers on damage and returns to full on its own when respawn refills the resource — refilling health *is* the signal, so there is no respawn-specific code in the UI. A brief flash on the bar marks the moment damage lands, which is now the player's damage-taken feedback (the character hit flash was removed as unworkable on the project's shaders). It reads the composed object's health resource and its `ValueChanged` facts and never mutates gameplay. **Deliberate placeholder:** drawn with IMGUI so it needs no canvas, prefab, or art; replace it with a real HUD widget bound to the same resource when a UI system arrives.
- **Attacks never interrupt jumps or rolls (Milestone 2, Review Group 5C).** A swing cannot begin while the player is rolling or airborne; the press buffers and fires on landing / roll recovery, so a traversal move always keeps its commitment without the input being dropped.
- **Player hit reactions disabled (Milestone 2, Review Group 5C — intentional decision).** When the player takes damage there is **no flinch animation, no hit flash, and no camera shake**; movement continues uninterrupted. Health still changes normally and enemy attacks still function — this is **presentation only** (`PlayerAnimatorDriver._playerHitReactions`, default false). The flinch both interrupted movement and produced a screen cue that read like the player's *own* attack connecting, muddying the impact signal. Re-enable once damage-taken readability is designed in its own right.
